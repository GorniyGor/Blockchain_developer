...
contract TokenSender {
    ...
    // ---- corrected contract version

    // New mapping
    mapping(bytes32 => bool) executed;

    // Add the nonce parameter
    function transferCorrected(
        address sender, uint amount, address recipient, address tokenContract, uint nonce, bytes memory signature
    ) public {
        // Pass ahead the nonce
        bytes32 messageHash = getHashCorrected(sender, amount, recipient, tokenContract, nonce);
        bytes32 signedMessageHash = messageHash.toEthSignedMessageHash();
    ...
    // Add the nonce parameter
    function getHashCorrected(address sender, uint amount, address recipient, address tokenContract, uint nonce) public pure returns (bytes32) {
